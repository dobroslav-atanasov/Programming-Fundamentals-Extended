namespace ArrayManipulator
{
    ...
    public class StartUp
    {
        ...
        private static string FirstOrLast(int[] array, string commandFirstOrLast, int count, string commandEvenOrOdd)
        {
            ...
            var type = commandEvenOrOdd == "odd" ? 1 : 0;
            var filter = array.Where(x => x % 2 == type).ToArray();

            return commandFirstOrLast == "first"
                ? $"[{string.Join(", ", filter.Take(count))}]"
                : $"[{string.Join(", ", filter.Reverse().Take(count).Reverse())}]";
        }

        private static string MaxOrMin(int[] array, string commandMinOrMax, string evenOrOdd)
        {
            var type = evenOrOdd == "odd" ? 1 : 0;
            var filter = array.Where(x => x % 2 == type).ToArray();

            if (!filter.Any())
                return "No matches";

            return commandMinOrMax == "min"
                ? Array.LastIndexOf(array, filter.Min()).ToString()
                : Array.LastIndexOf(array, filter.Max()).ToString();
        }
